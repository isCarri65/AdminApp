...
export const ModalInfoAdaptable = <T,>({
  ...
  useEffect(() => {
    if (objeto) {
      ...
      const propiedadesFiltradas = propiedadesIniciales.filter(
        (values) =>
          ![
            "id",
            "eliminado",
      ...
      const propiedadesEditadas = propiedadesFiltradas.map((arreglo) =>
        Object.keys(textoPropiedades).includes(arreglo[0])
          ? [textoPropiedades[arreglo[0]], arreglo[1]]
          : [arreglo[0], arreglo[1]]
      );
      ...
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        {objeto ? (
          propiedades.map(([key, value]) => (
            <div>
              {/*Condicional 1*/}
              {typeof value === "object" && key !== "alergenos" && value !== null ? (
                <div>
                  <p>
                    <strong>Domicilio:</strong> {value.calle} {value.numero}
                  </p>
                  <p>
                    <strong>Edificio:</strong> Piso {value.piso}, Depto {value.nroDpto}
                  </p>
                  <p>
                ...
                <div className={styles.logoContainer}>
                  ...
                  <div>
                    <img className={styles.img} src={value} alt={`${propiedades[0][1]} logo`} />
                  </div>
                </div>
              ) : key === "Logo" && value === null ? (
                <div className={styles.logoContainer}>
              ...
              ) : key === "alergenos" && value !== null ? (
                <div>
                  <strong>{key}: </strong>
                  <p>
                    {value.length > 0 ? value.map((e) => e.denominacion) : "No Posee Alergenos"}
                  </p>
                </div>
              ) : (
                <p>
